...
In `R` these are stored as `character` or `factor` (with `ordered = TRUE`).
...
In `R` these are stored as `numerical`, of which there are `integer` and `double` classes.
...
In `R` these are stored as `numerical`, of which there are `integer` and `double` classes.
`double` is more usualy but integer is useful sometimes, for example for ages.
