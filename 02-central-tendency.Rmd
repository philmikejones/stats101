...
```{r average-meme, cache=TRUE, fig.cap="What if I told you... there's more than one type of average"}
knitr::include_graphics("images/average-meme.jpg")
```
...
For example, in our `food` data set the variable `A121r` is the tenure type of each respondent.
A crosstab of this variable shows that response level `3` is the most frequent, which corresponds to owned tenure type.

```{python tenure-mode}
print(pd.crosstab(food["A121r"], "count"))
```
...
```{python food-income-median}
print(food.P344pr.median())
```

Which looks like this when we plot it as a distribution:

```{r distribution-income, fig.cap="Distribution of simulated income data", cache=TRUE}
...
```{r distribution-skewed, cache=TRUE}
